...
function findRequired(files, moduleInfo, cb) {
  var firstPass = false;
  ...
  examine({
    ...
    filterRequires(report, function () {
      ...
      // dig deeper
      findRequired(newRequiredPaths, moduleInfo, function (err, moduleInfo) {
        if (err) {
          return cb(err);
        }
        if (firstPass) {
...
function examine(options, cb) {
  (function (Path, fs, async, u) {
    ...
    function followRequired() {
      ...
      findRequired(options.files, function (err, moduleInfo) {
        if (err) {
          return cb(err);
        }
