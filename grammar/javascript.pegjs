...
MultiLineComment
  = "/*" chars:(!"*/" SourceCharacter)* "*/" {return '/*' + chars.map(function(a){return a.join('');}).join('') + '*/';}

MultiLineCommentNoLineTerminator
  = "/*" (!("*/" / LineTerminator) SourceCharacter)* "*/"

SingleLineComment
  = "//" chars:(!LineTerminator SourceCharacter)* {return '//' + chars.map(function(a){return a.join('');}).join('');}

SingleLineCommentGroup
  = SingleLineComment (WhiteSpace* LineTerminator WhiteSpace* SingleLineComment)*
...
CommentedStatement
  = __empty comments:(Comment __empty)* statement:Statement {
...
/*
StatementList
  = head:Statement tail:(__empty comments:(Comment __empty)* Statement)* {
      var result = [head];
      for (var i = 0; i < tail.length; i++) {
        result.push(addComment(tail[i][2], tail[i][1]));
      }
      return result;
    }
*/
...
FormalParameterList
  = p:Pos head:Identifier tail:(__ "," __ Identifier)* {
      var result = [head];
      for (var i = 0; i < tail.length; i++) {
        result.push(tail[i][3]);
      }
 ...
 */
SourceElement
  = __empty comments:(Comment __empty)* node:Statement {
